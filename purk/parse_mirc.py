...
def transform_bbersirc(start, end):
    if 'foreground' in end and 'background' in end and (
        ...
        result['background'] = end['background']
        return "\x04%s,%s" % (
            end['foreground'][1:], end['background'][1:]), ',', result
    else:
        return '', '', start
...
def transform(start, end, nextchar=" "):
    ...
    for f in transform_functions:
        for string, badchars, s in candidates[:]:
            ...
            string += newstring
            if newstring and (result is None or len(string) < len(result)):
...
if __name__ == "__main__":
    ...
    def parsed_eq(xxx_todo_changeme, xxx_todo_changeme1):
        (tags1, text1) = xxx_todo_changeme
        (tags2, text2) = xxx_todo_changeme1
        return setify_tags(tags1) == setify_tags(tags2) and text1 == text2

    def parsed_to_unparsed(xxx_todo_changeme2):
        (tags, text) = xxx_todo_changeme2
        result = []
        for i, char in enumerate(text):
            result.append(
                (dict(
                    tag['data'] for tag in tags if tag['from'] <= i < tag['to']),
                    char))
        return result
