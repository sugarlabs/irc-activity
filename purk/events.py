...
def unload(name):
    ...
    for e_name in list(all_events):
        for e_stage in list(all_events[e_name]):
            ...
            all_events[e_name][e_stage] = [
                (f, m) for f, m in to_check if m != s_name]
...
def onCommandLoad(e):
    ...
    try:
        if load(name):
            ...
            raise CommandError(
                "The script is already loaded; use /reload instead")
    except:
...
def onCommandReload(e):
    ...
    try:
        if reload(name):
            ...
            raise CommandError(
                "The script isn't loaded yet; use /load instead")
    except:
        e.window.write(traceback.format_exc(), line_ending='')
