...
def script_completer(window, left, right, text):
    return iter(list(core.events.loaded.keys()))


def network_completer(window, left, right, text):
    return iter(list(conf.get('networks', {}).keys()))
    ...
    while True:
        ...
        for cand in candidates:
            if cand.lower().startswith(text.lower()):
                window.input.text, window.input.cursor = insert_text % cand, cursor_pos + \
                    len(cand)
                yield None
