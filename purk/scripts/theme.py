...
# copied pretty directly from something that was probably copied from wine
# sources
...
def RGBtoHSL(r, g, b):
    ...
    if maxval == minval:
        ...
        if luminosity <= 120:
            saturation = (
                (maxval + minval) // 2 + delta * 240) // (maxval + minval)
        else:
            saturation = (
                (150 - maxval - minval) // 2 + delta * 240) // (150 - maxval - minval)
...
def HSLtoRGB(hue, saturation, luminosity):
    if saturation != 0:
        if luminosity > 120:
            mid2 = saturation + luminosity - \
                (saturation * luminosity + 120) // 240
        else:
        ...
        return tuple(
            (huetoRGB(hue + x, mid1, mid2) * 255 + 120) // 240 for x in (80, 0, -80))
    else:
...
def onHighlight(e):
    ...
    for word in conf.get('highlight_words', []) + \
            [e.network.me] + e.network.nicks:
...
def onText(e):
    ...
    if e.network.me == e.target:    # this is a pm
        ...
        if e.window.id == e.network.norm_case(e.target):
            ...
            to_write += "\x02<\x0F%s:%s\x0F\x02>\x0F " % (
                format_source(e), e.target)
        to_write += e.text
...
def onOwnText(e):
    ...
    if e.window.id == e.network.norm_case(e.target):
        ...
        to_write += "%s->\x0F \x02*\x0F%s\x0F\x02*\x0F %s" % (
            color, e.target, e.text)
...
def onOwnNotice(e):
    color = getsourcecolor(e)
    to_write = "%s-> \x02-\x02%s\x0F\x02-\x0F %s" % (
        prefix(e), e.target, e.text)
...
def onCtcp(e):
    color = getsourcecolor(e)
    to_write = "%s\x02[\x02%s\x0F\x02]\x0F %s" % (
        prefix(e), format_source(e), e.text)
...
def onJoin(e):
    if e.source == e.network.me:
        to_write = "%s%s %sjoin %s" % (
            prefix(e), format_info_source(e), address(e), e.target)
    else:
        to_write = "%s%s %sjoins %s" % (
            prefix(e), format_info_source(e), address(e), e.target)
...
def onPart(e):
    if e.source == e.network.me:
        to_write = "%s%s leave %s%s" % (
            prefix(e), format_info_source(e), e.target, text(e))
    else:
        to_write = "%s%s leaves %s%s" % (
            prefix(e), format_info_source(e), e.target, text(e))
...
def onKick(e):
    if e.source == e.network.me:
        to_write = "%s%s kick %s%s" % (
            prefix(e), format_info_source(e), e.target, text(e))
    else:
        to_write = "%s%s kicks %s%s" % (
            prefix(e), format_info_source(e), e.target, text(e))

    e.window.write(
        to_write,
        (e.target == e.network.me and widgets.HILIT) or widgets.EVENT)


def onMode(e):
    if e.source == e.network.me:
        to_write = "%s%s set mode:\x0F %s" % (
            prefix(e), format_info_source(e), e.text)
    else:
        to_write = "%s%s sets mode:\x0F %s" % (
            prefix(e), format_info_source(e), e.text)
...
def onNick(e):
    ...
    if e.source == e.network.me:
        ...
        to_write = "%s%s%s is now known as %s" % (
            prefix(e), color, e.source, e.target)
...
def onTopic(e):
    if e.source == e.network.me:
        to_write = "%s%s set topic:\x0F %s" % (
            prefix(e), format_info_source(e), e.text)
    else:
        to_write = "%s%s sets topic:\x0F %s" % (
            prefix(e), format_info_source(e), e.text)
    ...
    if not e.quiet:
        if e.msg[1].isdigit():
            if e.msg[1] == '332':
                ...
                if len(e.msg) == 6 and not e.msg[
                        4].isdigit() and not e.msg[5].isdigit():
                    e.window.write(
                        "* %s %s %s" %
