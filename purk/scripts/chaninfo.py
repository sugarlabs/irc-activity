...
def sortkey(network, channelname, nick):
    ...
    return '%s%s' % (''.join(str(int(mode not in nickmodes))
                             for mode in chanmodes), network.norm_case(nick))
...
def getchan(network, channel):
    return hasattr(
        network,
        'channels') and network.channels.get(
        network.norm_case(channel))
            ...
            if char == '+':
                ...
                if char in user_modes:
                    ...
                    if mode_on:
                        ...
                        channel.nicks[nickname] = channel.nicks[
                            nickname].replace(char, '')
