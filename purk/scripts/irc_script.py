...
def setdownRaw(e):
    if not e.done:
        ...
        if e.msg[1] == "PING":
            ...
            e.channel = e.target
            e.requested = e.network.norm_case(
                e.channel) in e.network.requested_joins
            ...
            e.channel = e.target
            e.requested = e.network.norm_case(
                e.channel) in e.network.requested_parts
            e.text = ' '.join(e.msg[3:])
            ...
            if e.network.norm_case(e.msg[3]) in e.network.requested_joins:
                e.network.requested_joins.discard(
                    e.network.norm_case(
                        e.msg[3]))
...
def onDisconnect(e):
    ...
    if hasattr(e.network, 'connect_timestamp'):
        if e.error and conf.get('autoreconnect', True):
            delay = time.time() - \
                e.network.connect_timestamp > 30 and 30 or 120
            ...
            def do_announce_reconnect():
                if not e.network.status:
                    ...
                    e.network._reconnect_source = ui.register_timer(
                        ...
                        1000,
                        do_reconnect)
            e.network._reconnect_source = ui.register_idle(
                do_announce_reconnect)
    ...
    if not e.done:
        if e.name in needschan and e.window.is_channel():
            ...
            if len(e.args) > chan_pos:
                if not e.args[chan_pos] or e.args[
                        chan_pos][0] not in valid_chan_prefixes:
